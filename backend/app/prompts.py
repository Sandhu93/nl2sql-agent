...
def _build_few_shot_prompt() -> ChatPromptTemplate:
    ...
    system_prompt = (
        ...
        "to answer it.\n\n"
        "LIMIT rules: Use LIMIT 1 when the question identifies a single winner/leader — "
        "e.g. 'who has the most not-outs', 'which batter has the most ducks', "
        "'which team has the highest win %', 'what is the highest/best/lowest'. "
        "Use LIMIT {top_k} ONLY when the user explicitly asks for multiple results: "
        ...
        "- SCORES/STATS GROUPED BY OPPONENT: when the question asks for a player's scores "
        "'against each team' or 'by opponent', GROUP BY bowling_team (the fielding team). "
        "batting_team is the player's OWN team and must never be aliased as 'opponent'. "
        "Example: SELECT bowling_team AS opponent, SUM(batsman_runs) AS runs FROM deliveries "
        "WHERE batsman = 'X' GROUP BY bowling_team ORDER BY runs DESC.\n"
        ...
        "- VENUE/STADIUM NAMES: Venue names in the matches table include the city suffix "
        "(e.g. 'Narendra Modi Stadium, Ahmedabad', 'Wankhede Stadium, Mumbai'). "
        "When a user provides a partial stadium name, use ILIKE '%partial_name%' rather than "
        "exact equality — e.g. WHERE venue ILIKE '%Narendra Modi%'. "
        "Only use exact equality when you can confirm the full venue string from context.\n"
        ...
        "WHERE year = 2025. It does NOT reflect current squad rosters, player transfers, "
        "retirements, or auction results. "
        "'Current team' queries should use the most recent playing_xi entry "
        "(ORDER BY year DESC, match_id DESC LIMIT 1). Never imply real-time accuracy.\n\n"
        "PLAYER NAME RESOLUTION:\n"
        "- Player names in deliveries/playing_xi are abbreviated short forms (e.g. 'B Sai Sudharsan', 'RG Sharma').\n"
        "- When a player's abbreviated name is already established from context or a previous query, "
        "filter deliveries.batsman or deliveries.bowler DIRECTLY using that short name. "
        "Do NOT re-resolve via the players table (player_full_name join) when the short name is known — "
        "the full name in players may differ from what was given, causing empty results.\n"
        "- Only use the players table join when you are uncertain of the short name and must look it up.\n\n"
        "DATA LIMITATIONS — awards and stats NOT stored in this database:\n"
        "- 'Player of the Series' / 'Player of the Tournament': NOT stored. "
        "The database only records 'player_of_match' (per-match award). "
        "If asked for Player of the Series, do NOT approximate using player_of_match frequency. "
        "Instead generate: SELECT 'Player of the Series data is not available in this database' AS note;\n"
        "- 'Orange Cap' (most runs in a season): NOT a stored column — derive it by "
        "SELECT batsman, SUM(batsman_runs) JOIN matches WHERE year=N GROUP BY batsman ORDER DESC LIMIT 1.\n"
        "- 'Purple Cap' (most wickets in a season): NOT a stored column — derive it from deliveries "
        "filtered by year and dismissal_kind.\n"
        "- 'Man of the Match', 'Player of the Match': USE player_of_match column in matches table.\n"
        "- DRS review decisions, player auction prices, salary caps, player nationalities: NOT stored.\n"
        ...
        "and let the database return empty results if none exist. "
        "Correct usage example (missing column): "
        "SELECT 'Player of the Series data is not available in this database' AS answer;\n\n"
        "Relevant cricket domain rules for this query:\n{cricket_context}\n\n"
        "Relevant table schema:\n{table_info}\n\n"
        "OUTPUT FORMAT: Return ONLY the raw SQL query. No explanations, no markdown fences (```sql), "
        "no prose before or after, no notes. Your response must start directly with SELECT or WITH.\n\n"
